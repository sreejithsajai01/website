...
export default function Admissions() {
  ...
  const activeContent = activePath ? componentMap[activePath] : <AdmissionProcess />;
            ...
            <aside className="lg:w-72 shrink-0">
              <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden sticky top-28">
                <div className="bg-primary px-5 py-4">
                  <p className="text-white font-bold uppercase tracking-wider text-sm">Admissions</p>
                </div>
                <nav className="flex flex-col">
                  {subPages.map((page) => {
                    const isActive = (activePath ?? "/admissions/process") === page.path;
                    return (
                      <Link
                        key={page.path}
                        href={page.path}
