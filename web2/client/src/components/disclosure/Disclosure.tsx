...
export default function Disclosure() {
  ...
  const activeContent = activePath ? componentMap[activePath] : <GeneralInformation />;
            ...
            <aside className="lg:w-72 shrink-0">
              <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden sticky top-28">
                <div className="bg-primary px-5 py-4">
                  <p className="text-white font-bold uppercase tracking-wider text-sm">Public Disclosure</p>
                </div>
                <nav className="flex flex-col">
                  {subPages.map((page) => {
                    const isActive = (activePath ?? "/disclosure/general-information") === page.path;
                    return (
                      <Link
                        key={page.path}
                        href={page.path}
